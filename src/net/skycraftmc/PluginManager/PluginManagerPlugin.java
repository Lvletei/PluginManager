...
public class PluginManagerPlugin extends JavaPlugin
{
	...
	private final CmdDesc[] help = {
		...
		new CmdDesc("plm list [options]", "Lists plugins with specified options, use -option to show options", "pluginmanager.list"),
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
	{
		if(args.length >= 1)
		{
			if(args[0].equalsIgnoreCase("list"))listCmd(sender, args);
			else if(args[0].equalsIgnoreCase("enable"))enableCmd(sender, args);
			else if(args[0].equalsIgnoreCase("disable"))disableCmd(sender, args);
			...
			else if(args[0].equalsIgnoreCase("load"))loadCmd(sender, args);
			else if(args[0].equalsIgnoreCase("unload"))unloadCmd(sender, args);
			else if(args[0].equalsIgnoreCase("reload"))reloadCmd(sender, args);
			else if(args[0].equalsIgnoreCase("softreload") || args[0].equalsIgnoreCase("sreload"))sreloadCmd(sender, args);
			else if(args[0].equalsIgnoreCase("show"))showCmd(sender, args);
			else
			{
				return msg(sender, ChatColor.GOLD + "Command unrecognized.  Type " + ChatColor.AQUA + "/plm" + ChatColor.GOLD + " for help");
			}
		}
		else helpCmd(sender, args, "PluginManager", help);
		return true;
	}
	...
	private boolean helpCmd(CommandSender sender, String[] args, String title, CmdDesc[] help)
	{
		...
		for(int i = 0; i < help.length; i ++)
		{
			...
			if(c.getPerm() != null)
			{
				if(!sender.hasPermission(c.getPerm()) && sender != getServer().getConsoleSender())continue;
			}
			if(d.size() < 10)
			{
				if(i >= (page - 1)*10 && i <= ((page - 1)*10) + 9)d.add((sender instanceof Player ? "/" : "") + c.asDef());
			}
